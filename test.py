...
def main():
    ...
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default="/home/tina/Documents/pipe_blaser/pipe_condition_assessment/riad/output/mvtec/leather/seed_6021/leather_2023-05-26-8974_model.pt",
    )
    parser.add_argument(
        "--grayscale",
        action="store_true",
    ...
    parser.add_argument("--seed", type=int, default=8490)
    ...
    plt.plot(
        fpr, tpr, label="%s pixel_ROCAUC: %.3f" % (args.obj, per_pixel_rocauc)
    )
    plt.legend(loc="lower right")
    save_dir = (
        args.save_dir
        ...
        + f"seed_{args.seed}"
        + "/"
        + "pictures_{:.4f}".format(threshold)
    )
    os.makedirs(save_dir, exist_ok=True)
    plt.savefig(os.path.join(save_dir, args.obj + "_roc_curve.png"), dpi=100)

    plot_fig(
        args, test_imgs, recon_imgs, scores, gt_mask_list, threshold, save_dir
    )
...
def plot_fig(args, test_img, recon_imgs, scores, gts, threshold, save_dir):
    ...
    for i in range(num):
        ...
        fig_img.savefig(
            os.path.join(save_dir, args.obj + "_{}_png".format(i)), dpi=100
        )
        plt.close()
