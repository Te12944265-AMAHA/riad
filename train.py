...
def main():
    ...
    parser.add_argument("--obj", type=str, default="leather")
    parser.add_argument("--data_type", type=str, default="mvtec")
    parser.add_argument(
        "--data_path",
        type=str,
        default="/home/tina/Documents/pipe_blaser/pipe_condition_assessment/riad/data/mvtec_ad",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--lr", type=float, default=0.0001, help="learning rate of Adam"
    )
    parser.add_argument(
        "--weight_decay", type=float, default=0.00001, help="decay of Adam"
    )
    ...
    log = open(
        os.path.join(
            args.save_dir, "model_training_log_{}.txt".format(args.prefix)
        ),
        "w",
    )
    ...
    train_dataset = MVTecDataset(
        args.data_path, class_name=args.obj, is_train=True, resize=args.img_size
    )
    ...
    val_loader = torch.utils.data.DataLoader(
        val_data, batch_size=32, shuffle=False, **kwargs
    )
    ...
    save_name = os.path.join(
        args.save_dir, "{}_{}_model.pt".format(args.obj, args.prefix)
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print_log(
            " {:3d}/{:3d} ----- [{:s}] {:s}".format(
                epoch, args.epochs, time_string(), need_time
            ),
            log,
        )
        ...
        if epoch % 10 == 0:
            save_sample = os.path.join(
                args.save_dir, "{}-images.jpg".format(epoch)
            )
            save_sample2 = os.path.join(
                args.save_dir, "{}test-images.jpg".format(epoch)
            )
            save_snapshot(
                x_normal_fixed,
                x_test_fixed,
...
def train(args, model, epoch, train_loader, optimizer, log):
    ...
    for data, _, _ in tqdm(train_loader):
        ...
        inputs = [
            data * (torch.tensor(mask, requires_grad=False).to(device))
            for mask in Ms
        ]
    ...
    print_log(
        "Train Epoch: {} L2_Loss: {:.6f} GMS_Loss: {:.6f} SSIM_Loss: {:.6f}"
        .format(epoch, l2_losses.avg, gms_losses.avg, ssim_losses.avg),
        log,
    )
...
def save_snapshot(x, x2, model, save_dir, save_dir2, log):
    ...
    with torch.no_grad():
        ...
        print_log(
            "Saved real and fake images into {}...".format(save_dir2), log
        )
